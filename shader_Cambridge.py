...
def judge_dist_and_deg(q1, t1, q2, t2):
    ...
    is_less_than_m = e_t <= 5
    is_less_than_deg = e_R < 10
...
if __name__ == "__main__":
    ...
    #-------------------------------------------------------------------------------------
    query_name_camera_first = dict()
    query_image_camera_param_path = "data/Cambridge/KingsCollege/query_images-camera_first.txt"
    with open(query_image_camera_param_path, 'r') as file:
        lines = file.readlines()

    for line in lines:
        parts = line.strip().split()
        image_name = parts[0]
        camera_first_params = float(parts[1])
        query_name_camera_first[image_name] = camera_first_params
    #-------------------------------------------------------------------------------------------
  

    query_image_list = "data/Cambridge/KingsCollege/results_sparse.txt"
    with open(query_image_list, "r") as f:
        for data in f.read().rstrip().split("\n"):
            ...
            last_folder = name.split('_')[0]
            image_name_in_seq = name.split('_')[1]
            base_image_folder = "/root/autodl-tmp/ace/datasets/Cambridge_KingsCollege"
            ...
            cam_intrinsic_first = query_name_camera_first[name]  ######
            cam_intrinsic_second = 512
            ...
            while OPEN:
                ...
                for pos_item, value_item in [(0, 1e-6), (0, -1e-6), (1, 1e-6), (1, -1e-6), (2, 1e-6), (2, -1e-6), (3, 1e-6), (3, -1e-6),
                                             (4, 1e-6), (4, -1e-6), (5, 1e-6), (5, -1e-6), (6, 1e-6), (6, -1e-6)
            ...
            # 打开文件以追加方式写入
            with open('data/Cambridge/KingsCollege/results_refine.txt', 'a') as f:
