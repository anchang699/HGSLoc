...
def calculate_h(qandt):
    ...
    with torch.no_grad():  # 禁用梯度计算，因为在渲染过程中不需要梯度信息
        ...
        for idx in range(len(scene.getTrainCameras())):
            ...
            ssim_value = ssim(rendering, query_image_tensor)
            diff_sum_all.append(1 - ssim_value.item())

            # psnr_value = psnr(rendering, query_image_tensor)
            # diff_sum_all.append(100 - psnr_value.mean().item())
...
def judge_dist_and_deg(q1, t1, q2, t2):
    ...
    is_less_than_m = e_t <= 5     ##########  根据定位评估结果确定
    is_less_than_deg = e_R < 20      ###########
...
if __name__ == "__main__":
    ...
    query_image_list = "data/db/playroom/results_sparse.txt"
    with open(query_image_list, "r") as f:
        for data in f.read().rstrip().split("\n"):
            ...
            image_name_in_seq = name
            base_image_folder = "/root/autodl-tmp/Hierarchical-Localization/datasets/db/playroom"

            query_image_path = base_image_folder + "/" + image_name_in_seq
            ...
            ##### 重点关注第一个参数
            cam_intrinsic_first = 1040.00730376
            cam_intrinsic_second = 1040.19275667
            ...
            while OPEN:
                ...
                for pos_item, value_item in [(0, 1e-2), (0, -1e-2), (1, 1e-2), (1, -1e-2), (2, 1e-2), (2, -1e-2), (3, 1e-2), (3, -1e-2),
                                             (4, 1e-1), (4, -1e-1), (5, 1e-1), (5, -1e-1), (6, 1e-1), (6, -1e-1)
            ...
            # 打开文件以追加方式写入
            with open('data/db/playroom/results_refine.txt', 'a') as f:
