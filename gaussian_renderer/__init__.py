...
def render(viewpoint_camera, pc: GaussianModel, pipe, bg_color: torch.Tensor, scaling_modifier=1.0, override_color=None):
    ...
    # 计算视场的 tan 值/焦距切线，这将用于设置光栅化配置。
    tanfovx = math.tan(viewpoint_camera.FoVx * 0.5)  # Field/Focal of View
    ...
    means3D = pc.get_xyz
    means2D = screenspace_points  # 设置全为0
    opacity = pc.get_opacity

    # If precomputed 3d covariance is provided, use it. If not, then it will be computed from
    # scaling / rotation by the rasterizer.
    # 如果需要预先计算的3D协方差矩阵，则计算它。否则，它将由光栅化器根据尺度和旋转进行计算。
    scales = None
    rotations = None
    cov3D_precomp = None
    ...
    shs = None
    colors_precomp = None
    if override_color is None:  # override:覆写
        if pipe.convert_SHs_python:
            ...
            # 将RGB颜色的范围限制在0到1之间
            # torch.clamp_min( , 0)函数将所有小于等于0的值裁剪为0
    ...
    # Those Gaussians that were frustum(视锥体) culled or had a radius of 0 were not visible
    # 那些被视锥体剔除或半径为0的高斯函数是不可见的。
