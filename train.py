...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    tb_writer = prepare_output_and_logger(dataset)  # 设置日志记录器和TensorBoard写入器。
    ...
    for iteration in range(first_iter, opt.iterations + 1): #主要的训练循环开始。       
        ...
        while network_gui.conn != None:
            try:
                ...
                # 接收 GUI 发送的消息。
                # 自定义相机设置、是否执行训练、是否转换球谐函数、是否计算三维协方差矩阵、保持连接、缩放修正因子
        ...
        # viewpoint_stack是视点栈/相机视角栈，其实是列表类型
        if not viewpoint_stack:  # 如果栈为空
            viewpoint_stack = scene.getTrainCameras().copy()
        # 生成的索引包括0和len(viewpoint_stack)-1
        ...
        if (iteration - 1) == debug_from:
            pipe.debug = True
        # 如果参数是随机背景颜色那么就设置随机颜色，否则如果背景是白色就设置为白色，否则设置为黑色
        ...
        with torch.no_grad():  # 记录损失的指数移动平均值，并定期更新进度条。
            # Progress bar
            ema_loss_for_log = 0.4 * loss.item() + 0.6 * ema_loss_for_log  # 累积的总损失
            if iteration % 10 == 0:
            ...
            if iteration < opt.densify_until_iter:  # 在达到指定的迭代次数之前执行以下操作。
                ...
                if iteration > opt.densify_from_iter and iteration % opt.densification_interval == 0:
                    # 根据当前迭代次数设置密集化的阈值。如果当前迭代次数大于opt.opacity_reset_interval(不透明度重置的间隔)，则设置 size_threshold 为 20，否则为 None。
...
# 准备输出目录并设置日志记录，然后创建Tensorboard写入器
def prepare_output_and_logger(args):    
    if not args.model_path:
        if os.getenv('OAR_JOB_ID'):
            unique_str = os.getenv('OAR_JOB_ID')
        else:
            unique_str = str(uuid.uuid4())  # 生成一个随机的唯一字符串, 也就是输出结果的文件夹的命名
        args.model_path = os.path.join("./output/", unique_str[0:10])
        
    # Set up output folder
    # 创建输出文件夹
    print("Output folder: {}".format(args.model_path))
    os.makedirs(args.model_path, exist_ok=True)  # 创建输出文件夹，如果文件夹已存在则不做任何操作。
    # 将参数 args 写入到model_path/cfg_args的文件中
    with open(os.path.join(args.model_path, "cfg_args"), 'w') as cfg_log_f:
        # 将参数 args 转换为字典，并使用 Namespace(**vars(args)) 将其初始化为一个命名空间对象，然后将其转换为字符串并写入到文件中。
        cfg_log_f.write(str(Namespace(**vars(args))))

    # Create Tensorboard writer
    tb_writer = None
    if TENSORBOARD_FOUND:
        tb_writer = SummaryWriter(args.model_path)  # 创建一个 SummaryWriter 对象，指定输出路径为 args.model_path。
    else:
...
if __name__ == "__main__":
    ...
    lp = ModelParams(parser)  # 来源于arguments/__init__.py文件中的ModelParams类
    op = OptimizationParams(parser)  # 来源于arguments/__init__.py文件中的OptimizationParams类
    pp = PipelineParams(parser)  # 来源于arguments/__init__.py文件中的PipelineParams类
    parser.add_argument('--ip', type=str, default="127.0.0.1")
    parser.add_argument('--port', type=int, default=6009)
    parser.add_argument('--debug_from', type=int, default=-1)
    parser.add_argument('--detect_anomaly', action='store_true', default=False)  # 检测异常`
    # parser.add_argument("--test_iterations", nargs="+", type=int, default=[3_000, 5_000, 7_000, 9_000, 11_000, 13_000, 15_000, 17_000, 19_000, 21_000, 23_000, 30_000])
    # parser.add_argument("--save_iterations", nargs="+", type=int, default=[3_000, 5_000, 7_000, 9_000, 11_000, 13_000, 15_000, 17_000, 19_000, 21_000, 23_000, 30_000])
    parser.add_argument("--test_iterations", nargs="+", type=int, default=[7_000, 30_000])
    parser.add_argument("--save_iterations", nargs="+", type=int, default=[7_000, 30_000])
    parser.add_argument("--quiet", action="store_true")  # 用于控制程序是否以安静模式运行
    ...
    # 初始化系统状态
    safe_state(args.quiet)  # from utils.general_utils import safe_state
